r"""
v5/monsooncast/dashboard/06_build_dashboard.py  —  STEP 6: build the React dashboard on
the 791 LGD units, styled after the v5 MonsoonCast frontend (dark theme with a
light toggle, gradient KPI cards, grouped layer toolbar, district-level
choropleth, click-through district briefing, sortable/searchable table).

TWO BUILDS FROM ONE COMPONENT
  The same App is emitted twice, so neither can drift from the other:

    MonsoonCast_LGD.html  a single self-contained file.  React, ReactDOM, Babel
                          and SheetJS come from a CDN and the data is inlined,
                          so it opens by double-clicking with no toolchain and
                          no server.  This is the one to email.

    dashboard_react/      a real Vite + React project: npm install, npm run dev.
                          React and XLSX are proper imports, the data is a
                          module import rather than a window global, and the
                          component is a normal default export.  This is the one
                          to develop in.

  An earlier version wrote only src/App.jsx with no package.json, index.html or
  entry point around it, which meant it could not actually be run -- the file
  existed but there was no project.  write_react_app() now emits the whole
  scaffold, including .vscode/ launch and task definitions.

OUTPUT -> v5/dashboard_lgd/MonsoonCast_LGD.html
          v5/dashboard_react/          (complete, runnable Vite project)

Run:  py -3.13 -X utf8 "v5/monsooncast/dashboard/06_build_dashboard.py"   (after step 05)
"""
import json
import pathlib
import shutil
...
OUT = HERE.parents[1] / "dashboard_lgd"
REACT = HERE.parents[1] / "dashboard_react"
V5DATA = HERE.parents[1] / "data_lgd"
sys.path.insert(0, str(HERE.parents[1]))
from common_v5 import log  # noqa

DEV_PORT = 5180          # distinct from the older v5 dashboard on 5175

CSS = r"""
:root{--bg:#0b1220;--panel:#111a2e;--panel-2:#0e1728;--edge:rgba(255,255,255,.08);
 --ink:#eef2fb;--ink-2:#9fb0cc;--ink-3:#6b7c9c;--accent:#37b6ff;--grid:#1c2842;
 --good:#22c55e;--warn:#f6b73c;--hot:#ff5a4d;--cool:#3b82f6;--mid:#243149;
 --nodata:#182338;}
:root[data-theme=light]{--bg:#eef2f8;--panel:#fff;--panel-2:#f4f7fb;
 --edge:rgba(11,18,32,.10);--ink:#0b1220;--ink-2:#47566f;--ink-3:#8492ab;
 --accent:#1f8fe0;--grid:#e2e8f2;--mid:#e7edf5;--nodata:#eef1f6;}
...
.top{position:sticky;top:0;z-index:40;background:linear-gradient(180deg,var(--bg) 72%,transparent);padding:16px 0 10px;display:flex;align-items:center;gap:14px;flex-wrap:wrap}
.brand{display:flex;align-items:center;gap:10px}
.logo{width:34px;height:34px;border-radius:9px;background:linear-gradient(135deg,var(--accent),#7b5cff);display:flex;align-items:center;justify-content:center;font-size:19px}
.brand h1{font-size:19px;margin:0;font-weight:700;letter-spacing:-.2px}
.brand .tag{font-size:11.5px;color:var(--ink-3);margin-top:1px}
.issue{margin-left:auto;font-size:12px;color:var(--ink-2);display:flex;gap:14px;align-items:center;flex-wrap:wrap}
.issue b{color:var(--ink)}
.tbtn{border:1px solid var(--edge);background:var(--panel);color:var(--ink-2);border-radius:8px;padding:6px 12px;cursor:pointer;font:inherit;font-size:12px}
.kstrip{display:grid;grid-template-columns:repeat(auto-fit,minmax(165px,1fr));gap:10px;margin:6px 0 16px}
.kpi{background:linear-gradient(180deg,var(--panel),var(--panel-2));border:1px solid var(--edge);border-radius:13px;padding:13px 15px}
.kpi .l{font-size:11px;color:var(--ink-3);text-transform:uppercase;letter-spacing:.4px}
.kpi .v{font-size:25px;font-weight:700;margin-top:3px;line-height:1.05}
.kpi .n{font-size:11px;color:var(--ink-2);margin-top:3px}
.toolbar{display:flex;gap:10px;align-items:center;background:var(--panel);border:1px solid var(--edge);border-radius:13px;padding:10px 12px;margin-bottom:16px;flex-wrap:wrap}
.grp{display:flex;gap:5px;align-items:center}
.lab{font-size:11px;color:var(--ink-3);text-transform:uppercase;letter-spacing:.4px;margin-right:2px}
.lbtn{border:1px solid var(--edge);background:var(--panel-2);color:var(--ink-2);border-radius:8px;padding:7px 12px;cursor:pointer;font:inherit;font-size:12.5px;transition:.12s}
.lbtn:hover{color:var(--ink)}
.lbtn.on{background:var(--accent);color:#04121f;border-color:var(--accent);font-weight:600}
.sep{width:1px;height:24px;background:var(--edge);margin:0 3px}
.main{display:grid;grid-template-columns:1.3fr 1fr;gap:16px;margin-bottom:16px}
...
.card h2{font-size:14px;margin:0 0 3px;font-weight:650;display:flex;align-items:center;gap:8px}
.card .cap{font-size:11.5px;color:var(--ink-3);margin-bottom:12px}
.dot{width:8px;height:8px;border-radius:50%;background:var(--accent)}
.mapwrap{position:relative}
.map{width:100%;height:auto;display:block}
.map path{stroke:var(--panel);stroke-width:.35;cursor:pointer;transition:.1s}
.map path:hover{stroke:var(--accent);stroke-width:1.4}
.map path.selp{stroke:var(--ink);stroke-width:1.6}
.legend{display:flex;align-items:center;gap:8px;font-size:11px;color:var(--ink-2);margin-top:10px;justify-content:center;flex-wrap:wrap}
.ramp{height:10px;width:200px;border-radius:5px;border:1px solid var(--edge)}
.search{width:100%;font:inherit;padding:8px 12px;border:1px solid var(--edge);border-radius:9px;background:var(--panel-2);color:var(--ink);margin-bottom:12px}
.dhead{display:flex;align-items:baseline;gap:8px}
.dhead .nm{font-size:19px;font-weight:700}.dhead .stt{font-size:12px;color:var(--ink-2)}
.dsub{font-size:11.5px;color:var(--ink-3);margin:2px 0 12px}
.hz{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px}
.hz .b{background:var(--panel-2);border:1px solid var(--edge);border-radius:10px;padding:10px;text-align:center}
.hz .b .h{font-size:10.5px;color:var(--ink-3)}
.hz .b .val{font-size:22px;font-weight:700;margin:2px 0}
.prow{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--grid);font-size:12.5px}
.prow .k{color:var(--ink-2)}.prow .v{font-weight:600;font-variant-numeric:tabular-nums}
.rk{display:flex;align-items:center;gap:8px;margin-bottom:5px;font-size:11.5px}
.rk .cn{width:120px;color:var(--ink-2);flex:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.rk .track{flex:1;height:11px;border-radius:6px;background:var(--grid);overflow:hidden}
.rk .fill{height:100%;border-radius:6px}
.rk .cv{width:44px;text-align:right;font-variant-numeric:tabular-nums;color:var(--ink-2)}
.scrolly{overflow-y:auto;max-height:430px}
table.tv{border-collapse:collapse;width:100%;font-size:12.5px}
table.tv th,table.tv td{padding:7px 9px;text-align:left;border-bottom:1px solid var(--grid)}
table.tv th{color:var(--ink-3);cursor:pointer;position:sticky;top:0;background:var(--panel);font-weight:600;font-size:11px;text-transform:uppercase;letter-spacing:.3px}
table.tv td.num,table.tv th.num{text-align:right;font-variant-numeric:tabular-nums}
table.tv tbody tr{cursor:pointer}table.tv tbody tr:hover{background:var(--panel-2)}
.tip{position:fixed;pointer-events:none;background:var(--panel);color:var(--ink);border:1px solid var(--edge);box-shadow:0 8px 22px rgba(0,0,0,.4);border-radius:9px;padding:8px 11px;font-size:12px;z-index:60;max-width:280px}
.tip b{display:block;margin-bottom:2px}
.note{font-size:11px;color:var(--ink-3);background:var(--panel-2);border:1px solid var(--edge);border-radius:9px;padding:9px 11px;margin-top:12px;line-height:1.55}
.foot{color:var(--ink-3);font-size:11px;margin-top:20px;line-height:1.6}
.star{color:var(--warn)}
"""

APP = r"""
const {useState, useMemo, useRef} = React;
const DATA = window.__DATA__, GEO = window.__GEO__;
...
# =====================================================================
# The browser build and the Vite build need different first and last lines --
# globals vs imports, a render call vs a default export -- and nothing else.
# Both markers are asserted before anything is written, so if the App source is
# ever edited in a way that moves them the build fails loudly here instead of
# emitting a project that silently will not start.
BROWSER_PRELUDE = ('const {useState, useMemo, useRef} = React;\n'
                   'const DATA = window.__DATA__, GEO = window.__GEO__;')
MODULE_PRELUDE = ('import React, {useState, useMemo, useRef} from "react";\n'
                  'import * as XLSX from "xlsx";\n'
                  'import DATA from "./data/data.json";\n'
                  'import GEO from "./data/geo.json";\n'
                  # the App reads DAILY off window in the browser build; give
                  # the module build the same global so one component serves
                  # both without a second code path
                  'import DAILY_JSON from "./data/daily.json";\n'
                  'if (typeof window !== "undefined") '
                  'window.__DAILY__ = DAILY_JSON;')
BROWSER_MOUNT = ('ReactDOM.createRoot(document.getElementById("root"))'
                 '.render(<App/>);')
MODULE_MOUNT = 'export default App;'

REACT_PKG = {
    "name": "monsooncast-lgd-dashboard",
    "private": True,
    "version": "5.1.0",
    "type": "module",
    "description": ("MonsoonCast — IMD gauge rainfall, crop stress and sowing "
                    "dynamics across 791 LGD districts"),
    #   No --port flag: vite.config.js already reads PORT from the
    #   environment and falls back to DEV_PORT. Repeating the port here as a
    #   command-line flag would OVERRIDE that, so an orchestrator assigning a
    #   free port would be ignored and the server would fail on a busy one.
    #   --open is dropped too: it launches a browser, which is wrong when the
    #   server is started by a preview harness that does its own navigation.
    "scripts": {"dev": "vite",
                "build": "vite build",
...
REACT_VITE = f"""import {{ defineConfig }} from 'vite'
...
export default defineConfig({{
  plugins: [react()],
  base: './',
  // PORT from the environment wins, {DEV_PORT} is only the fallback.
  //
...
<html lang="en">
  <head>
    ...
    <title>MonsoonCast — IMD/LGD district dashboard</title>
  </head>
  <body>
    <div id="root"></div>
...
# ${workspaceFolder} resolves to whichever folder VS Code has open, so these
# configurations work whether dashboard_react/ is opened directly or reached
# through the monsooncast.code-workspace at the project root.
REACT_LAUNCH = json.dumps({
    "version": "0.2.0",
    "configurations": [
        {"name": "MonsoonCast React — dev server",
         "type": "node-terminal", "request": "launch",
         "command": "npm run dev",
         "cwd": "${workspaceFolder}"},
        {"name": "MonsoonCast React — build",
         "type": "node-terminal", "request": "launch",
         "command": "npm run build",
         "cwd": "${workspaceFolder}"},
        {"name": "MonsoonCast React — open in Chrome",
         "type": "chrome", "request": "launch",
         "url": f"http://localhost:{DEV_PORT}",
         "webRoot": "${workspaceFolder}/src"},
    ]}, indent=2)

REACT_TASKS = json.dumps({
    "version": "2.0.0",
    "tasks": [
        {"label": "npm: install", "type": "shell", "command": "npm install",
         "options": {"cwd": "${workspaceFolder}"}, "problemMatcher": []},
        {"label": "npm: dev", "type": "shell", "command": "npm run dev",
         "options": {"cwd": "${workspaceFolder}"}, "problemMatcher": [],
         "isBackground": True,
         "group": {"kind": "build", "isDefault": True}},
        {"label": "npm: build", "type": "shell", "command": "npm run build",
         "options": {"cwd": "${workspaceFolder}"}, "problemMatcher": []},
    ]}, indent=2)
...
def to_module(app_src):
    """Browser App -> ES module App, or fail before writing anything."""
    if BROWSER_PRELUDE not in app_src:
        raise SystemExit("06: browser prelude not found in APP — the React "
                         "scaffold would be broken; fix BROWSER_PRELUDE")
    if BROWSER_MOUNT not in app_src:
        raise SystemExit("06: mount call not found in APP — the React scaffold "
                         "would have no entry point; fix BROWSER_MOUNT")
    m = app_src.replace(BROWSER_PRELUDE, MODULE_PRELUDE, 1)
    m = m.replace(BROWSER_MOUNT, MODULE_MOUNT, 1)
    return m


def write_react_app(app_src, css, daily=None):
    r"""Emit the Vite project.

    `daily` is the payload the STANDALONE build was given, already merged.
    It has to be passed in rather than re-read from disk: the CFSv2 layers
    are merged into that string in main(), not written back to
    daily_layers_lgd.json, so re-reading the file produced a React build with
    six fewer layers than the HTML one. Two builds from one component is the
    whole point of this file, and they had silently stopped matching.
    """
    src = REACT / "src"
    (src / "data").mkdir(parents=True, exist_ok=True)
    (REACT / ".vscode").mkdir(parents=True, exist_ok=True)

    (REACT / "package.json").write_text(json.dumps(REACT_PKG, indent=2),
                                        encoding="utf-8")
    (REACT / "vite.config.js").write_text(REACT_VITE, encoding="utf-8")
    (REACT / "index.html").write_text(REACT_INDEX, encoding="utf-8")
    (REACT / ".gitignore").write_text(REACT_GITIGNORE, encoding="utf-8")
    (REACT / ".vscode" / "launch.json").write_text(REACT_LAUNCH,
                                                   encoding="utf-8")
    (REACT / ".vscode" / "tasks.json").write_text(REACT_TASKS, encoding="utf-8")
    (src / "main.jsx").write_text(REACT_MAIN, encoding="utf-8")
    (src / "theme.css").write_text(css, encoding="utf-8")
    (src / "App.jsx").write_text(to_module(app_src), encoding="utf-8")

    # data as module imports rather than fetch(): the App reads DATA at module
    # scope, so a fetch would have to resolve before the module evaluates and
    # would need a loading shell around the whole component for no benefit here
    shutil.copyfile(OUT / "data.json", src / "data" / "data.json")
    shutil.copyfile(OUT / "districts.geojson", src / "data" / "geo.json")
    if daily is None:
        dp = V5DATA / "daily_layers_lgd.json"
        daily = dp.read_text(encoding="utf-8") if dp.exists() else "null"
    (src / "data" / "daily.json").write_text(daily, encoding="utf-8")
    #   Both builds must offer the same layers. If they diverge, say so here
    #   rather than letting one of them quietly ship without CFSv2.
    try:
        nl = len(json.loads(daily).get("layers", {}))
        log(f"  React daily payload: {nl} layers (same object as the "
            f"standalone build)")
    except Exception:
        pass
    mb = sum(f.stat().st_size for f in (src / "data").iterdir()) / 1e6
    log(f"  wrote dashboard_react/ (Vite project, {mb:.1f} MB of data, "
        f"dev server on :{DEV_PORT})")


def main():
    log("=" * 60)
    log("STEP 6 — build the React dashboard (v5-styled) on 791 LGD units")
    log("=" * 60)
    d = json.loads((OUT / "data.json").read_text(encoding="utf-8"))
    # The KPI means and every all-India / per-state figure now come from step
    # 05 inside data.json.  They used to be computed HERE and injected only into
    # the HTML's inlined payload, which is why the React project -- reading the
    # same file off disk -- showed "–%" on both KPI cards.  Anything derived
    # must go into data.json, never into one of the two builds.
    for k in ("agg", "mean_fc7", "mean_fc14"):
        if k not in d:
            raise SystemExit(f"06: data.json is missing '{k}' — re-run step 05 "
                             f"(05_forecast_export.py) before this one")
    data = json.dumps(d)
    geo = (OUT / "districts.geojson").read_text(encoding="utf-8")
    # Daily layers are optional: the dashboard degrades to monthly-only if
    # step 21 has not been run, rather than failing to build.
    dp = V5DATA / "daily_layers_lgd.json"
    daily = dp.read_text(encoding="utf-8") if dp.exists() else "null"
    if not dp.exists():
        log("  (no daily_layers_lgd.json — run features/21_daily_features.py "
            "for the daily view)")
    else:
        #   CFSv2 rides in the SAME payload under its own layer keys, so the
        #   map, slider, tooltip and export all reach it through one code path
        #   instead of a parallel one that would drift. It is a choice in the
        #   toolbar, never a substitution: the IMD layers are untouched.
        #
        #   Dates are intersected rather than unioned. The two products end on
        #   different days, and a date present for one but not the other would
        #   put the slider on a day where half the toolbar goes blank.
        cp = V5DATA / "cfsv2_layers.json"
        if cp.exists():
            dj = json.loads(daily)
            cj = json.loads(cp.read_text(encoding="utf-8"))
            #   Step 26 already reindexed CFSv2 onto these exact axes, so the
            #   merge is a dict update and nothing has to be re-aligned here.
            #   The lengths are still checked: this payload is POSITIONAL, so
            #   an array of the wrong length does not raise, it just draws the
            #   wrong district. Verifying is two comparisons; not verifying is
            #   a map that looks fine and is wrong.
            nd, ni = len(dj["dates"]), len(dj["ids"])
            good = {k: v for k, v in cj.get("layers", {}).items()
                    if len(v) == nd and all(len(r) == ni for r in v)}
            wrong = set(cj.get("layers", {})) - set(good)
            if wrong:
                log(f"  ! CFSv2 layers dropped, wrong shape for this payload "
                    f"({nd} dates x {ni} districts): {sorted(wrong)}")
            if good:
                dj["layers"].update(good)
                for k in ("edge", "method"):
                    dj.setdefault(k, {}).update(
                        {j: w for j, w in cj.get(k, {}).items() if j in good})
                daily = json.dumps(dj, separators=(",", ":"))
                log(f"  CFSv2 merged into the daily payload: "
                    f"{len(good)} layers on the same {nd}-day axis")
        else:
            log("  (no cfsv2_layers.json — run features/26_cfsv2_layers.py "
                "to offer CFSv2 as a second product)")
...
<script>window.__GEO__={geo};</script>
<script>window.__DAILY__={daily};</script>
<script type="text/babel">{APP}</script>
</body></html>"""
    (OUT / "MonsoonCast_LGD.html").write_text(html, encoding="utf-8")
    mb = (OUT / "MonsoonCast_LGD.html").stat().st_size / 1e6
    log(f"  wrote MonsoonCast_LGD.html ({mb:.1f} MB, standalone)")
    write_react_app(APP, CSS, daily)
